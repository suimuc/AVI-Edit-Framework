...
class BasePipeline(torch.nn.Module):
    ...
    def load_models_to_device(self, model_names=[]):
        # print(f'self.vram_management_enabled: {self.vram_management_enabled}')
    ...
    def freeze_except(self, model_names):
        ...
        for name, param in self.named_parameters():
            if any(key in name for key in model_names):
                param.requires_grad_(True)     # ✅ 解冻
                train_names.append(name)
            else:
                param.requires_grad_(False)    # ✅ 冻结
                eval_names.append(name)

        # print("Trainable parameters:", train_names)
